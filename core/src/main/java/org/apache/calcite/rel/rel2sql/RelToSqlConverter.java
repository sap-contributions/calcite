...
public class RelToSqlConverter extends SqlImplementor
  ...
  private static SqlHint toSqlHint(RelHint hint, SqlParserPos pos) {
    if (hint.kvOptions != null) {
      return new SqlHint(pos, new SqlIdentifier(hint.hintName, pos),
          nodeListOf(pos, hint.kvOptions.entrySet().stream()
              .flatMap(
      ...
      return new SqlHint(pos, new SqlIdentifier(hint.hintName, pos),
          nodeListOf(pos, hint.listOptions.stream()
  ...
  private static SqlNodeList nodeListOf(SqlParserPos pos, List<SqlNode> nodes) {
    SqlNodeList result = new SqlNodeList(pos);
    nodes.forEach(n -> result.add(n));
    return result;
  }

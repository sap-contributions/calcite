...
public class RelMetadataQuery extends RelMetadataQueryBase {
  ...
  /**
   * Returns the
   * {@link BuiltInMetadata.FieldsTrimmable#areFieldsTrimmable()}
   * statistic.
   *
   * @param rel         the relational expression
   * @return true or false depending on whether the columns are unique, or
   * null if not enough information is available to make that determination
   */
  public @Nullable Boolean areFieldsTrimmable(RelNode rel) {
    for (;;) {
      try {
        return fieldsTrimmableHandler.areFieldsTrimmable(rel, this);
      } catch (MetadataHandlerProvider.NoHandler e) {
